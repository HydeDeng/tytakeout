<template>
    <div class="cate-page" ref="CategoryDetail" @scroll="scroll" :class="notScroll?'notScroll':''">
...
export default class CategoryDetail extends Vue {
    ...
    async getShoppingList (cb?: any, byCategoryId?:any) {
        try {
    ...
    onLoad () {
        if (this.offset<=this.totalPage) {
            setTimeout(() => {
                ...
                console.log("categoryIdcategoryIdcategoryId")
                console.log(this.categoryId)
                console.log("end---categoryIdcategoryIdcategoryId")                
                this.getShoppingList(null,this.categoryId);
            }, 1000)
        } else {
            this.finished = true
            this.shopListShow = false
