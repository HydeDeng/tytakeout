<template>
    <div class="search-result-page" ref="SearchResult" @scroll="scroll" :class="notScroll?'notScroll':''">
        <!-- <categoryHeader>分类详情</categoryHeader> -->
        <categoryHeader class="header-warp" @return="headerReturn">分类详情</categoryHeader>
...
import CategoryHeader from "@/components/Layout/CategoryHeader.vue";
...
// @ts-ignore
import { getCategorieList, searchRestaurantsAndFoods, getAddressDetail } from '@/service'
...
export default class SearchResult extends Vue {
    ...
    clearFilter () {
        ...
        this.finished = false
        this.shoplist = []
        this.searchRestaurantsAndFoods()
        console.log(111)
    }
    menuChange (value) {
        // console.log(value)
    ...
    async searchRestaurantsAndFoods (cb?: any, byName?:any) {
        try {
    ...
    onLoad () {
        if (this.offset<=this.totalPage) {
            setTimeout(() => {
                ...
                console.log("end---keyword")                
                this.searchRestaurantsAndFoods(null,this.keyword);
            }, 1000)
        } else {
            this.finished = true
            this.shopListShow = false
